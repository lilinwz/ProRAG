...
os.environ["WANDB_PROJECT"] = "RAG-StepGRPO"
...
if __name__ == "__main__":
    ...
    tokenizer = AutoTokenizer.from_pretrained(POLICY_MODEL_NAME, trust_remote_code=True, padding_side='left')
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token

    rm_tokenizer = AutoTokenizer.from_pretrained(REWARD_MODEL_PATH, trust_remote_code=True, padding_side='left')
    if rm_tokenizer.pad_token is None:
        rm_tokenizer.pad_token = rm_tokenizer.eos_token

    reward_model = AutoModelForSequenceClassification.from_pretrained(
        REWARD_MODEL_PATH, dtype=torch.bfloat16, trust_remote_code=True, num_labels=1
    )

    policy_model = AutoModelForCausalLM.from_pretrained(
        POLICY_MODEL_NAME, dtype=torch.bfloat16, trust_remote_code=True, attn_implementation="flash_attention_2"
    )
