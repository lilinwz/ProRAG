...
if __name__ == "__main__":
    ...
    policy_model = AutoModelForCausalLMWithValueHead.from_pretrained(POLICY_MODEL_NAME, torch_dtype=torch.bfloat16, trust_remote_code=True, peft_config=lora_config)
    
    reward_model = AutoModelForSequenceClassification.from_pretrained(REWARD_MODEL_PATH, torch_dtype=torch.bfloat16, num_labels=1)
    reward_model.eval()
    ...
    def get_reward_score(texts: List[str], reward_model, tokenizer, device) -> torch.Tensor:
        with torch.no_grad():
        ...
        for i in tqdm(range(0, len(data_indices), BATCH_SIZE)):
            ...
            for step in range(MAX_INTERACTION_STEPS):
                ...
                for j, response_tensor in enumerate(response_outputs):
                    ...
                    reward = get_reward_score([env.state+response_text], reward_model, rm_tokenizer, device)[0]
            ...
            for k in range(len(envs)):
                ...
                rewards_tensor = torch.zeros_like(full_response_tensor, dtype=torch.float)

                current_token_index = 0
                for i, response_step_tensor in enumerate(all_step_responses[k]):
                    step_length = len(response_step_tensor)
                    reward_position = current_token_index + step_length - 1 
                    rewards_tensor[reward_position] = all_step_rewards[k][i]
                    current_token_index += step_length
                ...
                if final_answer_match:
                    ...
                    final_reward_val = 5.0 * f1
                else:
                    final_reward_val = -2.0
                
                rewards_tensor[-1] = final_reward_val
                all_rewards_for_logging.append(final_reward_val)

                final_query_tensors.append(initial_query)
                final_response_tensors.append(full_response_tensor)
                final_rewards.append(rewards_tensor)
            
            if not final_query_tensors: continue
            
            max_response_len = max([r.size(0) for r in final_response_tensors])

            padded_responses = []
            padded_rewards = []

            for r, rw in zip(final_response_tensors, final_rewards):
                response_pad_len = max_response_len - r.size(0)
                padded_r = torch.nn.functional.pad(r, (0, response_pad_len), value=tokenizer.pad_token_id)
                padded_responses.append(padded_r)

                reward_pad_len = max_response_len - rw.size(0)
                padded_rw = torch.nn.functional.pad(rw, (0, reward_pad_len), value=0)
                padded_rewards.append(padded_rw)

            stats = ppo_trainer.step(final_query_tensors, padded_responses, padded_rewards)
    ...
    unwrapped_model.save_pretrained(PPO_OUTPUT_DIR)